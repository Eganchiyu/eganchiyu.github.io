...
_includes/      → navigation.html, footer.html（实际使用）
_posts/         → 18 篇博文
assets/css/     → main.css（~1790 行，完整设计系统）
assets/js/      → main.js（~230 行，主题/菜单/代码复制）
页面            → index.html, categories.html, tags.html, year-archive.html
```
...
| 类别 | 问题 | 影响 |
|------|------|------|
| 缺失页面 | 没有 404 页面 | 用户遇到死链接时体验差 |
| 缺失资源 | favicon.png 被引用但不存在 | 浏览器标签页显示默认图标 |
| 性能 | MathJax 在每个页面都加载 | 非数学页面浪费带宽 |
| 性能 | Google Fonts 外部依赖 | 增加 DNS 查询和加载时间 |
| 样式 | 分类/标签/归档页使用内联 `<style>` | 样式管理不统一 |
| 功能 | 完全没有搜索功能 | 内容发现效率低 |
| 功能 | 没有评论系统 | 缺少读者互动渠道 |
| SEO | 缺少 Open Graph 标签 | 社交平台分享无预览卡片 |
| SEO | 缺少 JSON-LD 结构化数据 | 搜索引擎理解不充分 |
| 无障碍 | skip-links.html 存在但未引用 | 键盘用户无法跳过导航 |
| 遗留 | 大量未使用的 minimal-mistakes 模板文件 | 增加维护成本 |
...
4. **优化性能与 SEO**：条件加载、本地字体、OG 标签、结构化数据等
...
- 方案 A（推荐）：在 Front Matter 中添加 `math: true` 字段，仅在该字段为 true 时加载 MathJax
  - 修改 `default.html`，用 `{% if page.math %}` 条件包裹 MathJax 相关代码
  - 需要数学渲染的文章在 Front Matter 中添加 `math: true`
- 方案 B：创建 `math.html` layout 继承 `single.html`，仅在该 layout 中引入 MathJax

**涉及文件**：
- 修改：`_layouts/default.html`（添加条件判断）
- 修改：需要数学渲染的文章的 Front Matter（添加 `math: true`）

**验收标准**：
- 包含 LaTeX 的文章正常渲染数学公式
- 不包含 LaTeX 的文章不加载 MathJax 脚本
- 页面加载速度有可感知的提升
