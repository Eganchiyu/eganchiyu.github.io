...
# 七、首次正式 Release：冻结内核，明确边界

在完成核心建模逻辑并经过多轮实验验证后，我为项目创建了 **首个 GitHub Release（v0.1.0）**。  
这一步的目的并不是“发布一个产品”，而是**主动冻结一个可被他人理解和复现的稳定内核版本**。

从工程角度看，这是一次非常关键的阶段性动作。
...
这种状态**不适合讨论、不适合展示、更不适合让老师或他人试用**。

创建 Release，本质上是在回答三个问题：
...
## 7.2 Release 前做了哪些清理工作？

在创建 v0.1.0 之前，我对仓库结构做了明确的边界划分：

- 将实验性脚本、无关文件统一迁移并通过 `.gitignore` 排除
    
- 删除早期误打的 tag，重新创建干净的版本标记
    
- 明确区分：
    
    - **核心逻辑（Release 包含）**
        
    - **实验 / 临时 /验证代码（Release 不包含）**
        

同时，我刻意接受了一个现实：

> **当前版本仍然使用“假题库 + 写死路径”进行演示。**

这是一个**有意识的取舍**，而不是疏忽。

---

## 7.3 为什么允许“假数据 + 写死路径”进入 Release？

这个 Release 的定位是：

> **核心建模逻辑的稳定版本，而非完整应用系统。**
...
> **Release 的是“方法论与结构”，不是“最终使用形态”。**

这也为下一阶段的工程化改造（配置文件、数据库、界面）  
预留了充分空间，而不会破坏已验证的内核。

---

## 7.4 Release v0.1.0 的明确定位

在 Release 描述中，我对版本做了明确声明：

- 类型：**Initial stable core release**
    
- 定位：**Research-oriented / teaching-grade prototype**
...
这一版 Release 并没有解决新的技术问题，但它完成了一件更重要的事：

> **第一次把项目从“进行中代码”变成了“可被引用的阶段性成果”。**
...
这也是为什么，**第七节写 Release，第八节写 Config 工程化，是非常自然的顺序**：
...
于是，我在这一步引入了一个非常关键的工程化改造：  
**配置驱动（Configuration-driven Modeling）**。

---

## 8.1 为什么一定要引入配置文件？

从系统设计角度看，这里至少有三类“变化频繁但不该改代码”的信息：
...
> **代码只负责“怎么解”，配置文件负责“要解什么”。**

---

## 8.2 为什么选 YAML？

我最终选择了 `config.yaml`，而不是 JSON 或 Python 配置，原因很现实：
...
`paper:   total_questions: 9  question_types:   - name: single_choice     count: 6   - name: fill_blank     count: 2   - name: proof     count: 1  difficulty:   target_average: 3   bucket_constraints:     1_star: 0.1     2_star: 0.2     3_star: 0.4     4_star: 0.2     5_star: 0.1`

这已经非常接近“老师能看懂”的程度了。

---

## 8.3 Config 类：配置与建模之间的“翻译层”

配置文件不能直接丢进 CP-SAT 使用，中间必须有一层**语义转换**。

于是我写了一个 `Config` 类，职责非常明确：

> **把“人类可读的配置”，转成“求解器可用的离散参数”。**
...
后续所有模块**不再关心配置文件结构**，只通过属性访问。

---

### （2）题型数量约束的标准化

`@property def type_limits(self):     return {t["name"]: t["count"] for t in self.data["question_types"]}`

把 YAML 中的列表结构，转成约束模块可以直接用的字典。
...
这一阶段改动，并没有提升算法性能，但**意义远大于性能优化**：

1. 系统第一次具备了“可用性”
    
2. 模型从“研究代码”迈向“工程系统”
    
3. 后续 GUI / Web / 批量生成 都有了统一入口
    
4. 老师的需求可以直接映射为配置，而不是改代码
